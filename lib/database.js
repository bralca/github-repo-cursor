...
export async function fetchGithubRawData(entityType, githubId) {
  if (dbType === 'sqlite') {
    ...
    const row = await db.get(
      'SELECT * FROM github_raw_data WHERE entity_type = ? AND github_id = ?',
      [entityType, githubId]
    );
...
/**
 * Store GitHub raw data in the database
...
export async function storeGithubRawData(record) {
  if (dbType === 'sqlite') {
    ...
    try {
      // For SQLite with auto-increment ID, don't include ID in the insert
      await db.run(
        `INSERT OR REPLACE INTO github_raw_data 
      ...
      console.error('Error storing data in SQLite:', err);
      return false;
    }
  } else {
    // Use Supabase
...
/**
 * Query GitHub raw data by entity type
...
export async function queryGithubRawData(entityType, options = {}) {
  const { limit = 10, offset = 0, orderBy = 'fetched_at', order = 'desc' } = options;
  ...
  if (dbType === 'sqlite') {
    ...
    try {
      ...
      console.error('Error querying data from SQLite:', err);
      return [];
    }
  } else {
    // Use Supabase
