...
export default async function RepositoryPage({ params }: RepositoryPageProps) {
  ...
  console.log(`[RepositoryPage Debug] Received repositorySlug: ${repositorySlug}`);
  
  // Extract the GitHub ID from the slug
  const slugInfo = parseRepositorySlug(repositorySlug);
  
  console.log(`[RepositoryPage Debug] Result of parseRepositorySlug:`, slugInfo);
  
  if (!slugInfo) {
    console.log(`[RepositoryPage Debug] No slug info extracted, returning 404`);
    notFound();
  }
  
  console.log(`[RepositoryPage Debug] Processing repository slug: ${repositorySlug}`);
  console.log(`[RepositoryPage Debug] Extracted GitHub ID: ${slugInfo.githubId}`);
  console.log(`[RepositoryPage Debug] Extracted name: ${slugInfo.name}`);
  
  // Fetch the repository data using our function
  try {
    console.log(`[RepositoryPage Debug] Calling getRepositoryByGithubId with ID: ${slugInfo.githubId}`);
    const repository = await getRepositoryByGithubId(slugInfo.githubId);
    
    console.log(`[RepositoryPage Debug] getRepositoryByGithubId response:`, repository);
    
    if (!repository) {
      console.log(`[RepositoryPage Debug] No repository data found for GitHub ID: ${slugInfo.githubId}`);
      notFound();
    }
    ...
    console.error('[RepositoryPage Debug] Error fetching repository data:', error);
